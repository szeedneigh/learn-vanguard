...
export default function Resources() {
  ...
  const createSubjectMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setTimeout(() => {
        logger.log("Final refetch after subject creation");
        refetchSubjects();
      }, 1000);
    onError: (error) => {
      toast({
  ...
  const deleteSubjectMutation = useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      // Add delayed refetches to ensure UI is updated
        logger.log("Delayed refetch after subject deletion (500ms)");
      }, 500);
        logger.log("Delayed refetch after subject deletion (1500ms)");
      }, 1500);
  ...
  useEffect(() => {
    ...
    if (currentSubjectId) {
      ...
      logger.log("Refetching data for subject:", currentSubjectId);
  ...
  const handleAddSubject = (result) => {
    ...
    if (result && result.name) {
      ...
      // Manually trigger refetch since we're not using createSubjectMutation
        logger.log("Final refetch after subject creation via AddSubjectModal");
      logger.error("Invalid result from AddSubjectModal:", result);
  };
  ...
  const handleDeleteAnnouncement = (announcementId) => {
    ...
    const idToDelete =
      typeof announcementId === "object" && announcementId._id
        ? announcementId._id
        : announcementId;
    logger.log(`Deleting announcement with ID: ${idToDelete}`);
  ...
  const handleSaveAnnouncement = (values) => {
    ...
    if (editingAnnouncement) {
      ...
      if (!announcementId) {
        ...
        logger.error("Missing announcement ID:", editingAnnouncement);
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {currentSubject ? (
          <ViewSubject
            ...
            refetchAll={() => {
              logger.log("ViewSubject: Triggering refetchAll");
              refetchResources();
              refetchTopics();
              refetchAnnouncements();
