...
export const getNavigationByRole = (user) => {
  if (!user || !user.role) {
    logger.log("getNavigationByRole: No user or role provided");
    return [];
  }
  // Normalize user role to uppercase for consistent comparison
  const normalizedUserRole = user.role.toUpperCase();
  logger.log("getNavigationByRole: User role normalized:", {
    original: user.role,
  ...
  return navigationConfig.filter((item) => {
    ...
    if (!hasRoleAccess) {
      logger.log(`Navigation item ${item.name} denied: role mismatch`, {
    ...
    if (!hasRequiredPermissions) {
      logger.log(`Navigation item ${item.name} denied: missing permissions`, {
