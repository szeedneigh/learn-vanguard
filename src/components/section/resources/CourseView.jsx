...
const CourseView = ({
  ...
  const currentYearObject = programData?.years?.find((y) => y.year === currentYear);
  ...
  return (
    <div className="space-y-6">
            ...
            <ScrollArea className="w-full" orientation="horizontal">
              <div className="flex space-x-2 pb-2">
                {currentYearObject.semesters.map((semesterString) => (
                  <Button
                    key={semesterString}
                    variant={currentSemester === semesterString ? "default" : "outline"}
                    onClick={() => {
      ...
      {currentYearObject && currentSemester && (
        subjectsLoading ? (
          ...
          <Alert variant="destructive" className="mt-4">
            ...
            <AlertDescription>
              {subjectsError?.message || "An unexpected error occurred while fetching subjects."}
            </AlertDescription>
          </Alert>
          ...
          <div className="text-center py-8">
            <p className="mt-4 text-muted-foreground">
              {searchTerm ? `No subjects found matching "${searchTerm}".` : "No subjects found for the selected criteria. Select a program, year, and semester, or try a different search."}
            </p>
          </div>
        )
      )}
